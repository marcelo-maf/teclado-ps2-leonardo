 ...
 * Data: 18/02/2026 - Versão 1.9.2 (Base 1.6 + Filtro Zero Fantasma)
 */
...
void enviarTeclaBruta(uint8_t tecla, bool shift) {
  KeyReport_t report;
  report.reserved = 0;
  report.modifiers = shift ? 0x02 : 0x00; 
...
void loop() {
  if (dataReady) {
    ...
    switch (code) {
      ...
      // --- FILTRO DO ZERO FANTASMA ---
      case 0x45: 
        static unsigned long ultimaVezDoZero = 0;
        // Só escreve o '0' se tiver passado 150ms desde a última vez
        if (millis() - ultimaVezDoZero > 150) {
          Keyboard.write('0');
          ultimaVezDoZero = millis();
        }
        break; 

      case 0x16: Keyboard.write('1'); break;
      ...
      case 0x51: 
        if (shiftAtivo) {
          Keyboard.press(KEY_RIGHT_ALT); Keyboard.write('w'); Keyboard.release(KEY_RIGHT_ALT);
        } else {
          Keyboard.write(0xDC); 
        }
        break;
